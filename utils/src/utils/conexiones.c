...
void enviarBufferPorPaquete(t_buffer *buffer, uint8_t codigo_de_operacion, int un_socket) {
	...
	// Por último enviamos
	send(un_socket, aEnviar, buffer->size + sizeof(uint8_t) + sizeof(uint32_t), 0);
...
t_paquete *recibirPaqueteGeneral(int un_socket) {
	...
	recv(un_socket, &(paquete->codigoOperacion), sizeof(uint8_t), 0);
...
uint32_t leerDeBufferUint32(t_buffer * buffer) {
	uint32_t valor;
	memcpy(&valor, buffer->stream, sizeof(uint32_t));
...
uint8_t leerDeBufferUint8(t_buffer * buffer) {
	uint8_t valor;
	memcpy(&valor, buffer->stream, sizeof(uint8_t));
...
char * leerDeBufferString(t_buffer * buffer, uint32_t tamanio_string) {
	...
	memcpy(buffer->stream, string, tamanio_string); 
	return string;
}